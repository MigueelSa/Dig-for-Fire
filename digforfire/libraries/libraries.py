...
class Spotify(Library):
    ...
    def _feed_release(self, data: AlbumData) -> AlbumData:
        ...
        artist = [
            artist.get("name") for artist in artist_credit if isinstance(artist, dict)
        ]
        # album
        title = data.get("name", "")
...
class MusicBrainz(Library):
    ...
    def _fetch_library(
        ...
        for ialbum, album in enumerate(
            tqdm(
                self.local_library,
                desc="Enriching library with MusicBrainz...",
                leave=False,
                mininterval=5.0,
            )
        ):
            ...
            if local_id in existing_albums:
                continue
            artist, name = album.get("artist", []), self._normalize_title(
                album.get("title", "")
            )
            ...
            try:
                ...
                if result["release-list"]:
                    ...
                    full = mb.get_release_by_id(
                        release_id,
                        includes=["tags", "release-groups", "artist-credits", "media"],
                    )
                    time.sleep(1)
                ...
                album_data = album.copy()
                logging.error(
                    f"Failed to retrieve MusicBrainz data for {artist}-{name}: {e}",
                    exc_info=True,
                )
    ...
    def add_album(self, title: str, artist: str) -> None:
        ...
        try:
            ...
            if result["release-list"]:
                ...
                logging.error(
                    f"No release found on MusicBrainz for {artist} – {title}."
                )

        except WebServiceError as e:
            logging.error(
                f"Failed to retrieve MusicBrainz data for {artist}-{title}: {e}",
                exc_info=True,
            )
